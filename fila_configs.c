...
// Função para abrir o arquivo da fila (usada pelos dois programas) OK
...
// Função para fechar ambos os arquivos (usada pelos dois programas) OK
void configs_fechar(FILE *arquivo) {
    fclose(arquivo);
}

// Função para criar a fila (usada pelo programa TV.C) OK
...
void atualizar_fila(filaEspera *fila, ficha *nova_ficha, int *senha_atual){
    ...
    if(nova_ficha->senha == *senha_atual){ // Caso a fila esteja vazia, não poderemos comparar com a senha do ultimo
        ...
        } else { // Se ficha não está na fila (verifica pelo último elemento)
